...
@recipe function f(subsampledplot::SubsampledPlot, execution::Execution{T,<:Simulation{T,M}}) where {T,M<:WCMSpatial}
    ...
    dt = subsampledplot.time_subsampler.Δ == nothing ? saved_dt(simulation) : subsampledplot.time_subsampler.Δ

    dx = subsampledplot.space_subsampler.Δ == nothing ? saved_dx(simulation) : subsampledplot.space_subsampler.Δ
