...
function NeuralModels.directed_weights(conn::R2S1IsotropicConnectivity{T},
    ...
    local_weights = directed_weights.(ExpSumSqDecayingConnectivity{T}, diffs_R2,
        conn.local_amplitude, Ref(conn.local_R2_spread), Ref(step_size[1:2]))
    S1_based_long_range_weights = directed_weights(ExpSumSqDecayingConnectivity{T}, diffs_S1,
        1.0, Ref(conn.long_range_S1_spread), Ref(step_size[[3]]))
    R2_based_long_range_weights = directed_weights(ExpSumSqDecayingConnectivity{T}, diffs_R2,
        long_range_amplitude, Ref(conn.long_range_R2_spread), Ref(step_size[1:2]))
