@EI_kw_example function example(N_ARR=2,N_CDT=2,P=2; sep=100.0, spread_scale=1.0, amplitude_scale=1.0, SNR_scale=80.0)
  simulation = Simulation(;
    model = WCMSpatial{Float64,N_ARR,N_CDT,P}(;
      ...
      nonlinearity = pops(SigmoidNonlinearity{Float64};
        a = [1.2, 1.0],
        θ = [2.6, 8.0]),
      stimulus = pops(MultipleDifferentStimuli{Float64,N_CDT},
          pops(GaussianNoiseStimulus{Float64,N_CDT}; SNR = [1.0, 1.0] .* SNR_scale),
          pops(MultipleSameStimuli{Float64,N_CDT,SharpBumpStimulus{Float64,N_CDT},2};
                strength = [1.2, 1.2],
                width = [28.1, 28.1],
